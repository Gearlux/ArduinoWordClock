...
void setupButtons()
{
  ...
  buttonOne.init(BUTTON_ONE_PIN, HIGH);
  buttonTwo.init(BUTTON_TWO_PIN, HIGH);
  buttonThree.init(BUTTON_THREE_PIN, HIGH);
  buttonFour.init(BUTTON_FOUR_PIN, HIGH);
}
    ...
    case automatic:
      {
        ...
        float scale = brightness_low + max(0.0f, min(1.0f, reading / 1024.0f)) * (brightness_high - brightness_low);
...
void change_hour(int h, int min5, int m, bool reset)
{
  last_change = millis();
  dt = clock.getDateTime();
  int hour = (int)dt.hour + h;
  if (hour > 23)
    hour = 0;
  if (hour < 0)
    hour = 23;    
  int min = dt.minute + min5 + m;
  if (min > 59)
    min = 60 - min;  
  if (min < 0)
    min = 60 + min;
