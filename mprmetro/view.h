...
public:
  ...
  void enableMinuteDots(uint8_t minute, const HsbColor &color, byte config);
...
private:
  void display_five_minutes(const HsbColor &color, uint8_t fiveMinutes, const LedWord &after, byte config, bool blink);
  void display_the_hour(const HsbColor &color, uint8_t hour_index, byte config, bool blink);
  ...
  void display_dots(const HsbColor &color, uint8_t minute, byte config, bool blink);
